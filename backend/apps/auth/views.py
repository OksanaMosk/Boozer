...
from apps.auth.services.auth_service import activate_user_by_token, reset_user_password, handle_social_user_data
...
class ActivateUserView(GenericAPIView):
    ...
    def activate_user(self, token):
        ...
        user, status_msg = activate_user_by_token(token)

        if status_msg == "already_active":
...
class RecoveryPasswordView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not token:
            raise ValidationError({'detail': 'Token is required.'})
        user = reset_user_password(token, serializer.validated_data['password'])
...
class SocialLoginJWTAPIView(GenericAPIView):
    ...
    def handle_social_login(self, provider, access_token):
        ...
        user, profile = handle_social_user_data(user_data['email'], user_data)
