...
export const BoozerVenuesClientComponent = () => {
    ...
    const renderStepContent = () => {
        ...
        if (step === 4 && confirmedOrderId && selectedVenue) {
            return <BoozerStep4TableSelectionComponent venueId={String(selectedVenue.id)} orderId={confirmedOrderId} onNext={handleStep4TableSubmit} onBack={() => setStep(3)} />;
        }
        if (step === 3 && confirmedOrderId && selectedVenue) {
            return <BoozerStep3MenuComponent venueId={String(selectedVenue.id)} orderId={confirmedOrderId} onNext={handleStep3MenuSubmit} onBack={() => setStep(2)} />;
        }
        if (step === 2 && selectedVenue) {
            return <BoozerStep2OrderBaseInfoComponent venueId={selectedVenue.id!} onBack={() => setStep(1)} onNext={handleStep2Submit} />;
        }
